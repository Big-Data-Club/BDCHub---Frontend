...
export const Step4Review: React.FC<Step4ReviewProps> = ({ form, onChange, errors, lang, onEditStep }) => {
  ...
  const reviewCard = "bg-white dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 shadow-sm dark:shadow-lg space-y-3";
  const reviewHeader = "flex justify-between items-center border-b border-slate-100 dark:border-slate-800 pb-3";
  const reviewTitle = "text-sm font-bold text-blue-600 dark:text-blue-400 flex items-center gap-2";
  const labelCls = "text-slate-400 dark:text-slate-400 block";
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      <div className="border-b border-slate-100 dark:border-slate-800 pb-4">
        ...
        <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">{t.step4Desc}</p>
      </div>
