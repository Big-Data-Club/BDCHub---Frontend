  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      ...
      <div>
        ...
        <p className="text-xs text-slate-400 mb-4">{t.deptSelectHint}</p>
            ...
            return (
              <div
                ...
                className={`group relative p-5 rounded-2xl border cursor-pointer transition-all duration-300 flex flex-col justify-between ${
                  isSelected
                    ? `bg-slate-900/90 border-blue-500 shadow-[0_0_30px_rgba(59,130,246,0.3)] ring-1 ring-blue-500/50`
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-xs font-medium text-blue-400/90 mt-1">
                ...
                <div className="mt-5 pt-3 border-t border-slate-800/80">
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {(lang === "vi" ? dept.skillsVi : dept.skillsEn).map((skill, idx) => (
                      <span
                        key={idx}
                        className="text-[11px] bg-slate-800/80 text-slate-300 px-2 py-0.5 rounded-md border border-slate-700/60"
                      >
                        {skill}
                      </span>
                    ))}
        ...
        {errors.department && <p className="text-xs text-rose-400 mt-2">{errors.department}</p>}
      </div>
      ...
      <div>
        ...
        <p className="text-xs text-slate-400 mb-2">{t.motivationHint}</p>
        <textarea
          rows={5}
        ...
        />
        {errors.motivation && <p className="text-xs text-rose-400 mt-1">{errors.motivation}</p>}
      </div>

      {/* Send Copy via Email Option */}
      <div className="p-4 bg-slate-900/70 border border-slate-800 rounded-xl flex items-center space-x-3">
        <input
          type="checkbox"
          id="sendCopy"
          checked={form.sendCopy}
          onChange={(e) => onChange({ sendCopy: e.target.checked })}
          className="w-4 h-4 rounded border-slate-700 bg-slate-900 text-blue-500 focus:ring-blue-500/30"
        />
        <label htmlFor="sendCopy" className="text-xs text-slate-300 font-medium cursor-pointer flex items-center gap-2">
