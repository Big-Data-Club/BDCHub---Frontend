              ...
              <div key={project.id}>
                <Link href={project.projectShowcaseUrl} className="block group">
                  <TerminalCard className="p-5">
                    ...
                    <p className="text-sm text-slate-600 dark:text-slate-400 mt-2">{project.desc}</p>
                  </TerminalCard>
                </Link>
              </div>
            ))}
        ...
        <div>
          ...
          <div className="space-y-6">
            {clubData.publications.map((pub) => (
              <div
                key={pub.id} 
                className="p-4 rounded-xl bg-white dark:bg-[#0F1E35] border border-slate-200 dark:border-blue-500/20 shadow-sm hover:border-blue-400/60 dark:hover:border-cyan-400/40 transition-all duration-300 group"
              >
