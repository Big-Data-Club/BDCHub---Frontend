...
export function CourseProgressAndGoalCard({
  ...
  if (loading) {
    return (
      <div className="h-[198px] w-full bg-white dark:bg-[#0F1E35] rounded-2xl border border-slate-200 dark:border-blue-500/10 animate-pulse" />
    );
  }

  return (
    <div className="bg-white/80 dark:bg-[#0F1E35]/80 backdrop-blur-xs border border-slate-200/85 dark:border-blue-500/15 rounded-2xl p-5 shadow-xs hover:border-blue-300 dark:hover:border-blue-500/25 transition-all duration-200 w-full grid grid-cols-1 md:grid-cols-[1fr_1px_1fr] gap-x-6 md:gap-x-8 gap-y-4 md:gap-y-3 relative">

      {/* 1. Left column: Title & Info (Col 1, Row 1) */}
      <div className="md:col-start-1 md:row-start-1 flex flex-col justify-end">
        ...
        {totalCount > 0 ? (
          <div className="h-2.5 w-full">
            ...
            <TooltipProvider delayDuration={55}>
              <div className="h-2.5 rounded-full overflow-hidden flex bg-slate-200 dark:bg-[#080F1E] border dark:border-blue-500/5">
                {completedCount > 0 && (
                  <UITooltip>
      ...
      <div className="md:col-start-1 md:row-start-3 w-full border-t border-slate-200/60 dark:border-blue-500/10 pt-3 min-h-[48px] flex items-center">
        {totalCount === 0 ? (
      ...
      <div className="hidden md:block md:col-start-2 md:row-start-1 md:row-span-3 w-px bg-slate-200 dark:bg-blue-500/15 self-stretch my-1 flex-shrink-0" />

      {/* 5. Right column: Title & Info (Col 3, Row 1) */}
      <div className="md:col-start-3 md:row-start-1 md:pl-2 flex flex-col justify-end">
        <div className="flex items-center justify-between gap-2">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-xl flex items-center justify-center bg-purple-50/80 text-purple-655 dark:bg-purple-950/60 dark:text-violet-400 border border-purple-200/50 dark:border-purple-500/20">
              <Target className="w-4 h-4" />
            </div>
            <div>
              <h4 className="text-sm font-bold text-slate-900 dark:text-white">Học phần tiếp theo</h4>
            </div>
          </div>

          {focusCourse && focusCourse.progress_percent !== undefined && (
            <span className="text-xs font-extrabold text-blue-650 dark:text-cyan-400 bg-blue-50/70 dark:bg-blue-900/30 px-2 py-0.5 rounded-full border border-blue-200/20 dark:border-blue-500/20">
        ...
        {focusCourse ? (
          <h3 className="text-sm font-extrabold text-slate-900 dark:text-white mt-2 line-clamp-1" title={focusCourse.course_title}>
      ...
      <div className="md:col-start-3 md:row-start-2 md:pl-2 flex items-center h-6">
        {focusCourse && focusCourse.progress_percent !== undefined && focusCourse.progress_percent > 0 ? (
          <TooltipProvider delayDuration={55}>
            <UITooltip>
              <TooltipTrigger asChild>
                <div className="h-2.5 bg-slate-200 dark:bg-[#0D192E] rounded-full overflow-hidden border dark:border-blue-500/5 w-full flex cursor-pointer group/progress">
                  <div
                    className="h-full bg-purple-600 dark:bg-purple-500 rounded-full transition-all duration-500 group-hover/progress:brightness-110 group-hover/progress:scale-y-125"
      ...
      <div className="md:col-start-3 md:row-start-3 md:pl-2 w-full border-t border-slate-200/60 dark:border-blue-500/10 pt-3 min-h-[48px] flex items-center">
        {focusCourse ? (
          <PrimaryBtn
            onClick={() => onNavigateToCourse?.(focusCourse.course_id)}
            className="w-full bg-none bg-purple-600 hover:bg-purple-700 text-white text-xs font-semibold py-2 rounded-xl transition-all duration-200 active:scale-[0.97] flex items-center justify-center gap-1.5 dark:bg-none dark:bg-purple-600 dark:hover:bg-purple-700 dark:text-white"
