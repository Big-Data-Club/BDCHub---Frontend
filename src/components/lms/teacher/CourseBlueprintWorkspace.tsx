...
export function CourseBlueprintWorkspace({ userId, organizations, onComplete, onCancel }: { userId: number; organizations: Organization[]; onComplete: (blueprint: CourseBlueprint) => Promise<void>; onCancel: () => void }) {
  ...
  const approve = async () => { if (!blueprint) return; setBusy("approve"); try { await onComplete(await courseBlueprintService.approve(blueprint.id, userId)); } catch (cause) { setError(cause instanceof Error ? cause.message : "Bạn cần hoàn tất các trường bắt buộc"); } finally { setBusy(null); } };
